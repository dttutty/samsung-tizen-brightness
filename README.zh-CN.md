...
安装配套 Tizen 应用后，程序会先唤醒电视网络与桥接服务，再直接调节亮度且不遮挡 HDMI。安装了 Tizen Studio 时，程序还会自动使用隐藏的 SDB 通道恢复桥接器，无需保持 Device Manager 打开。没有开发者模式时会自动退回模拟遥控器方式，调节过程中电视设置菜单会短暂出现。

电脑睡眠、关机或 HDMI 断开时，程序会退出电视端桥接器，让显示器恢复正常的 HDMI 无信号自动关机逻辑；唤醒或重新连接后会自动恢复桥接。若电脑突然断电，电视端桥接器也会在失去电脑连接 30 秒后自行退出。
